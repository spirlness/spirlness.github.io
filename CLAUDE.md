...
npm run content:check   # content-integrity suite alone (the on-edit hook's content leg)
npm run new-post -- <slug> --title "…"   # scaffold a validated MDX post in content/posts/
npm run test:e2e  # Playwright page tests against `serve out` (build first)
```
...
- **Internal navigation goes through `SmartLink` (`src/components/ui/SmartLink.tsx`)** — external links render a plain `<a target="_blank" rel="noopener noreferrer">`, `#anchor` links keep a plain `<a>`, and internal routes render `next/link` with `normalizeInternalHref()` guaranteeing the trailing slash. The old plain-`<a>`-everywhere rule was retired when the Playwright e2e suite (CI's `Run page tests` step, against `serve out`) took over re-verifying click-through navigation on the exported site — the escape hatch that rule always demanded. If e2e is ever removed or weakened, restore plain `<a>` for internal links.
...
**`content/updates/*.json`** feeds the homepage timeline, one file per entry. `date` is `"YYYY-MM"`; sorting splits it numerically rather than trusting engine `Date` parsing. `getAllUpdates()` **throws** on a malformed or schema-invalid file (zod validation in `src/lib/content-schemas.ts`), so a broken entry fails the build instead of silently vanishing from the homepage. Adding an icon name means editing two places: the `updateIcons` zod enum in `src/lib/content-schemas.ts` *and* the `updateIcons` JSX map in `src/app/page.tsx`. Current set: `award` | `book` | `graduation` | `project` | `publication` | `blog`.

**`content/projects/*.json`** each define one project; an optional same-name `.mdx` supplies the detail-page body, and its absence makes the page fall back to `description`. The route is keyed on the **`id` field inside the JSON**, and the `id` must equal the filename. `parseProject()` in `src/lib/content-schemas.ts` enforces that at build time (it throws on a mismatch), so an id/filename divergence fails the build rather than silently exporting a 404 page — the failure mode this used to rely on CI to catch. `getAllProjects()` validates the filename through `assertSafeContentSlug()`, and CI independently re-checks each `id` for URL-safety, for equality with its filename, and for its `out/projects/<id>/index.html`.
...
**MDX pipeline.** `getPostBySlug()` strips frontmatter with `gray-matter` first, then compiles the remaining body through `compileContent()` in `src/lib/mdx.ts` — the shared `compileMDX` (`next-mdx-remote/rsc`) entry with `remarkMath` + `rehypeKatex` + `rehype-pretty-code` (Shiki, `github-dark`, `keepBackground: false` so the custom dark `pre` styling stays) and `parseFrontmatter: false` (the frontmatter is already gone by that point). Posts pass `citations: true, tableOfContents: true`; `getProjectDetailById()` compiles with the same stack minus citations, so fenced code blocks highlight in project `.mdx` too. Frontmatter must supply `title`, `date`, `excerpt`; the filename becomes the slug. Optional frontmatter: `tags` (array or comma string; each tag must match `^[A-Za-z0-9-]+$` or the build fails — it becomes the `/blog/tag/<tag>/` route), `lang` (`en` | `zh`, default `en`; drives the post's `og:locale`, so a Chinese post must set `lang: zh`), and `lastUpdated` (shown beside the date). KaTeX CSS is imported globally in `src/app/layout.tsx`. Three authoring conventions:
...
- **Citations are `[@bibtexKey]`**, or `[@keyA; @keyB]` for several at once. The `citationPlugin` remark plugin in `src/lib/mdx.ts` (enabled only when posts compile with `citations: true`) rewrites them in place into numbered sup-style links and collects the cited entries in first-appearance order for the `<References>` block. It visits only mdast `text` nodes, so a citation inside a code span or fence is left as literal text (a documented way to *show* the syntax). An unknown key **throws and fails the build**, so dead citations cannot reach production. Keys may only contain `[A-Za-z0-9:-]` — `src/lib/bibtex.ts` enforces that same charset at parse time (via `CITATION_KEY_REGEX` in `src/lib/content-id.ts`, which `citationPattern` is also built from), so a key like `smith_2020` or a keyless `@article{, …}` fails the build rather than quietly rendering as literal text in every post that cites it. Projects compile without the plugin, so a `[@key]` inside a project `.mdx` renders as literal text.
...
- **The `pre` override is `CodeBlock`, a client wrapper** (`src/components/mdx/CodeBlock.tsx`) that keeps the dark-block `<pre>` styling and adds a hover copy button. It must stay client-only (it calls `navigator.clipboard`); `MDXComponents.tsx` imports it like any other client component.
- **The Distill-style ToC lives in the left gutter.** `rehype-slug` assigns the heading `id`s and `collectHeadingsPlugin()` in `src/lib/mdx.ts` records them onto `post.headings` (both run only when posts compile with `tableOfContents: true`); the client component `TableOfContents` (`src/components/mdx/TableOfContents.tsx`) renders them `sticky` in the gutter and scrollspies via `IntersectionObserver`. It is gated `min-[1400px]:` for the same reason as `SideNote` — below that the gutter is too narrow and the nav would overflow — so on smaller screens the post simply has no ToC.
...
**Validation.** `src/lib/content-integrity.ts` (`npm run content:check`, also the on-edit hook's content leg and CI's "Validate content" step) is the one place that ties the readers together: it compiles every post and project, rejects duplicate BibTeX keys, rejects orphan project `.mdx` files, and checks every local destination the content declares — post and project MDX bodies, `siteProfile.navLinks`, update `link` fields, project `links` and `thumbnail`. MDX bodies are parsed by `extractLocalTargets()` in `src/lib/content-targets.ts` with the same remark stack the site compiles with (`remark-parse` + `remark-gfm` + `remark-math` + `remark-mdx`), **not** by matching source text: that is what keeps fenced and inline code out of the results, separates `![img]()` from `[link]()`, and resolves nested and reference-style links the way the browser will. Destinations are then split by shape — one with a file extension resolves to a file under `public/` and must exist there (path traversal out of `public/` is rejected), everything else is normalized with `normalizeInternalHref()` and must be a known route (`/`, `/blog/`, `/projects/`, `/publications/`, every post, project, and tag page, plus the generated `/feed.xml`, `/robots.txt`, `/sitemap.xml`). A JSX `<a href="…">` / `<img src="…">` attribute is checked too; an expression-valued one (`href={x}`) is not.

## Claude Code automation

`.claude/` is committed (only `settings.local.json` and `worktrees/` are gitignored). Four pieces are wired:

- **`.claude/hooks/on-edit.mjs`** — a PostToolUse hook on `Edit|Write` (`.claude/settings.json`). After an edit it runs `eslint --fix --max-warnings 0` on the touched `.ts/.tsx/.js/.mjs` file, the matching `src/lib/__tests__/<name>.test.ts` when a `src/lib/*.ts` file changes, and the content-integrity suite when anything under `content/` changes. Consequences: **a just-edited file may be auto-fixed on disk — re-Read before editing it again**, and an exit-2 feedback message means there are problems left after the autofix; fix them, don't push past them. The script invokes the local eslint/vitest binaries via `process.execPath` (no shell, no npx) so it works identically on Windows and POSIX.
- **`/ship`** (`.claude/skills/ship/`) — the Deployment section's full ritual as one command: local gate → branch/commit/push → PR → (only on explicit instruction) merge → background-watch the Actions run → verify production + annotation-free.
- **`/verify-export`** (`.claude/skills/verify-export/`) — reproduces CI's "Verify static export" step against `out/`, then serves it and spot-checks routes in the browser.
- **`content-reviewer`** (`.claude/agents/`) — reviews a pending diff against this file's conventions (SideNote placement, `##` sections, citation keys, href normalization, no-op Tailwind classes, id==filename). Run it before committing content or MDX changes.
