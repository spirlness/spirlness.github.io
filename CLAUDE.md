...
- **The Distill-style ToC lives in the left gutter.** `collectHeadingsPlugin()` in `src/lib/posts.ts` is a per-call rehype plugin that slugs `h2`/`h3` into `id`s and records them on `post.headings`; the client component `TableOfContents` (`src/components/mdx/TableOfContents.tsx`) renders them `sticky` in the gutter and scrollspies via `IntersectionObserver`. It is gated `min-[1400px]:` for the same reason as `SideNote` — below that the gutter is too narrow and the nav would overflow — so on smaller screens the post simply has no ToC. Heading text must be slug-friendly (a heading that slugs to empty still gets `id="section"`).
